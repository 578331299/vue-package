<template>
  <div class="content_box">
    ...
    <div class="button-div">
      ...
      <el-button type="primary" @click="show_selection">导出</el-button>
    </div>
    ...
    >
      ...
      >
        ...
        <el-table-column prop="cpys" label="车牌颜色" align="center">
          <template scope="scope">{{$store.state.enumData.hpys[scope.row.cpys]}}</template>
        </el-table-column>
        ...
        <el-table-column prop="rlzl" label="燃料种类" align="center">
          <template scope="scope">{{$store.state.enumData.rlzl[scope.row.rlzl]}}</template>
        </el-table-column>
    ...
    <check-selection :diaSta="checkShow" @dialogClose="close_selection" :type="selectType"></check-selection>
...
export default {
  ...
  mounted() {
    // let key_ = sessionStorage.getItem("key_data");
  ...
  methods: {
   ...
   showPdfDialog(data, name) {
    ...
    log_search() {
      ...
      this.$axios
        .get("/serveApi/hbj/remoteSensing/getPage", {
          params: {
