    ...
    >
      <div style="height:calc(100% - 45px);overflow: auto">
        ...
        >
          ...
          <el-table-column prop="sfxx" label="是否违规行驶" align="center">
            <template slot-scope="scope">
              <a
                :style="{color:scope.row.sfxx == 1 ? '#909399' : 'red'}"
              >{{scope.row.sfxx==1 ? "否" : "是"}}</a>
            </template>
          </el-table-column>
          <el-table-column prop="xxtj" label="是否提交交警" align="center">
            <template slot-scope="scope">
              <a
                :style="{color:scope.row.xxtj == 1 ? '#909399' : 'red'}"
              >{{scope.row.xxtj==1 ? "已提交" : "未提交"}}</a>
            </template>
          </el-table-column>
          <el-table-column label="车牌图片" align="center">
            <template slot-scope="scope">
              <el-button
...
import dubleSubDataView from "../../common/dubleSubDataView.vue";
import exportFile from "../../common/exportFile.vue";
export default {
  name: "SingleAndDoubleNumberLimit",
  components: {
    pictureBig,
    dubleSubDataView,
    exportFile
  ...
  data() {
    return {
      ...
      updataData: [
        ...
        { sbzxd: "是否违规行驶", check: false, name: "sfxx" }
      ],
      nameObj: {
        jlbh: "记录编号",
        dwmc: "点位名称",
  ...
  methods: {
    ...
    SubmitPolice(rowdata) {
      if (rowdata === null) {
        ...
        this.$refs.dubleSubDataViewDom.handleClose();
      } else {
        let arr = [rowdata.id];
    ...
    upload_excel(objData) {
      ...
      this.$axios({
        ...
        responseType: "blob",
        url: "/serveApi/hbj/remoteSensing/export",
        data: obj
      })
        .then(res => {
          ...
          a.href = href;
          a.download = "doubleNumberLimit.xls";
      ...
      // window.open("/serveApi/hbj/remoteSensing/exportExecl")
    },
    /**
     * @description 图片查看
     * @author lifei
    ...
    upload_pdf(subData) {
      ...
      this.$axios({
        ...
        .then(res => {
          ...
          downloadElement.download = "doubleNumberLimit.pdf";
